...
async function getEventAttendance(
  ...
  if (eventId) {
    const attendance = await eventAttendanceStore.listByUserAndEvent(
      userId,
      eventId,
    );
...
async function createEventAttendance(
  ...
  const attendance = await eventAttendanceStore.create(
    userId,
    eventId,
    attedanceCreate,
  );
...
async function updateEventAttendance(
  ...
  if (!attendance) {
    ...
    const newAttendance = await eventAttendanceStore.create(
      userId,
      eventId,
      context.req?.body,
    );
  ...
  const attendanceData = await eventAttendanceStore.update(
    attendance._id,
    userId,
    eventId,
    attendance,
