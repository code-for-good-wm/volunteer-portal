...
export const eventAttendancesSlice = createSlice({
  ...
  reducers: {
    ...
    attendancesReceived(
      state,
      action: PayloadAction<{ attendances: EventAttendance[] }>,
    ) {
      state.loading = false;
...
export const { attendanceAdded, attendancesReceived } =
  eventAttendancesSlice.actions;

export const {
  selectAll: selectAllAttendances,
  selectById: selectAttendanceById,
  selectIds: selectAttendanceIds,
} = eventAttendancesAdapter.getSelectors(
